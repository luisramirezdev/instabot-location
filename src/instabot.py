...
class InstaBot:
    ...
    def new_auto_mod_comments(self):
        if time.time() > self.next_iteration["Comments"] and self.comments_per_day != 0 \
                and len(self.media_found) > 0 \
                and self.check_exisiting_comment(self.media_found[0]['code']) == False:
            comment_text = self.generate_comment()
            log_string = "Trying to comment: %s" % (self.media_found[0]['id'])
            self.write_log(log_string)
            if self.comment(self.media_found[0]['id'], comment_text) != False:
